...
|---|---|---|
| Low-poly 3D house and orbit camera | Done | Mouse orbit and zoom; pan disabled. Keyboard orbit uses WASD/arrow keys and zoom uses Q/E, -/+, or numpad -/+. Impact camera shake is disabled when the operating system requests reduced motion. |
| Sims-style cutaway | Done | Exterior walls, full-height interior partitions, and doors use smooth angle/pitch/zoom opacity. Bedroom walls now extend to the wing wall height with matching infill above every doorway, removing the permanently open upper section. Both roof sections use the same pitch/zoom opacity function; the wing roof spans the full shared-wall run and tucks beneath the original eave, while its materials disable depth writes and omit a thresholded shadow. Production build and target-browser smoke testing pass. |
| Roomy 3BR/1BA furnished interior | Done | The wing is now deeper, giving every bedroom more floor area. The primary bed now rests against the bathroom-side wall and faces a dresser plus wall-mounted TV centered together on the opposite wall; its portable heater now sits in the open bedside strip instead of intersecting the bed silhouette. In both secondary bedrooms, the beds sit against the north wall and the dressers sit flush to their side walls, aligned beside the beds. The bathroom vanity is placed on the south wall with a compact countertop handle, a rectangular basin, and a small rectangular shutoff tab below it; no pipe or U-shaped control passes through the sink. The room also includes a mirror, storage, towels, bath mat, and hamper, while the bathtub has no exposed fixture. In the original core, the TV now sits on the west wall opposite a rotated sofa, a taller floor lamp and plant add vertical balance, and the sink sits close to the stove. A small framed State Farm poster hangs above the kitchen sink and follows the north wall's cutaway fade. Production build and target-browser smoke testing pass. |
| Hailstorm | Done | The result now states that golf-ball-sized hail damaged the roof; the National Weather Service defines that comparison as 1.75-inch hail. Dense deterministic hail strikes both roof sections, with the wing using the same staged dent, color-damage, prevention, and persistence logic as the core roof. The authored national demo uses a $12,000 unprotected repair estimate and a $3,500 reduced estimate: with the fixed $1,000 demo deductible, the panel derives potential insurer payments of $11,000 and $2,500, a $1,000 policyholder share, and $8,500 damage avoided by the $4,000 impact-resistant-roofing upgrade. These figures are illustrative scenarios rather than quotes because roof costs, product performance, coverage valuation, and wind/hail deductibles vary. Airborne hail is clipped at both roof surfaces, impact chips stay above their roof planes, and settled hail excludes the combined core-and-wing roof footprint. A brief, gentle camera rattle begins with the roof strikes; it remains deliberately lighter than the tree impact and is halved with impact-resistant roofing. Low-poly drifts remain outside the complete house. Production build and target-browser smoke testing pass. |
| Kitchen fire | Done | Stove-origin fire advances through counter, TV, fridge, living, dining, and the east wall; the result panel accurately explains that it spread to the furniture and charred the walls, matching the dense, staged trail of overlapping floor char, wall scorch, persistent stovetop soot, and warped burner. The unprotected fire keeps burning until reset. The shallow char geometry now has explicit clearance and depth offset above the finished kitchen floor so the slab cannot occlude it. Fire audio starts in the pre-paint phase and skips each clip's measured low-energy lead-in so its first cue lands with the first visible stove burst. With smoke detectors + extinguisher enabled, a procedural smoke alarm sounds while a red extinguisher enters, sweeps foam across the stovetop flame, and fades after the flame is out; only the localized reduced scorch remains. Production build and target-browser smoke testing pass. |
| Bathroom water loss | Done | Clicking the vanity shutoff immediately starts a deterministic under-sink burst aimed into the bathroom, with no trigger-time geometry mount. Three anchored core jets ease rapidly to full reach while 64 arcing droplets launch in a tight stagger. A dark, opaque pool starts growing in the open aisle on the first frame, followed by three overlapping pools with authored ease-out timing and a subtle settling ripple. The pools are shallow, floor-flush, irregular water surfaces that remain legible against the teal floor without transparent-surface sorting or artificial outline rings. The result panel opens after 2 seconds, once the initial burst and puddles establish, while spray and matching seeded water audio persist until reset. A visible leak sensor + automatic shutoff limits the spray and sound to 1.35 seconds, reduces their density, and keeps one clearly exposed puddle contained. Production build and target-browser smoke testing pass. |
| Electrical arc fire | Done | The primary-bedroom power strip visibly carries five occupied sockets feeding the TV, console, phone charger, floor lamp, and portable heater. Clicking only the strip starts deterministic blue-white arcs, then sequentially pops the connected devices, races into the wall circuit, and ignites a bounded cord-path bedroom fire with sparks, multi-source smoke, device shutdown, melted debris, and a broad persistent scorch trail. The unprotected procedural crackle loops with the persistent fire until reset. When smoke detectors are selected but AFCI protection is not, the smoke alarm starts with the first flame, not the initial arc, and continues without reducing the outcome. AFCI breakers + an electrical inspection add a visible protected breaker, trip the circuit at 0.48 seconds, shorten the procedural audio, and stop the event at a small singe before it spreads. Production build and target-browser smoke testing pass. |
...
| Insurance information panel | Done | Appears shortly after the event's key beat; shows a financial snapshot: estimated uninsured out-of-pocket exposure, potential insurer payment, and the $1,000 demo deductible. “Estimated” now labels the authored repair scenario while “Up to” is reserved for the possible insurer payment. Reduced and eliminated outcomes also quantify estimated damage avoided against the unprotected baseline. The disclaimer states that repair amounts are demo scenarios rather than quotes, identifies the replacement-cost and sufficient-limits assumptions, and notes that roof coverage and wind/hail deductibles vary. The outdated full-loss savings nudge has been removed, and a tip above “Got it” explains that acknowledging the result unlocks another disaster. The panel becomes a viewport-bounded scroller with a sticky acknowledgement action on narrow or short windows. Production build and target-browser smoke testing pass. |
| Prevention controls | Done | Prevention is snapped when an event begins, locks afterward, and affects visuals/panel/cost. Cross-disaster relationships lock too: the smoke alarm + extinguisher card becomes disabled when either the stove or power strip is triggered and stays locked until Reset house. The simulator presents each choice as a compact, high-contrast protection card with a visible checkbox, readable short label, and Active/Off/Locked state. The cards remain in one horizontally scrollable toolbar row so the house scene keeps its vertical space; the introductory setup provides the full risk-and-benefit explanation. Production build and target-browser smoke testing pass. |
| Savings-pot game | Done | Replaces the former Home Risk Score. The player starts with a $50,000 savings pot shown in a top-right meter (green → amber → red as it drains). Each resolved disaster subtracts its full uninsured repair estimate; when a bill exceeds the current savings, the balance shows the entire negative shortfall instead of stopping at $0. Selecting a protection subtracts its cost (roof $4,000, alarm $200, leak $800, tree $1,500, AFCI $1,200); unselecting it refunds that full cost. A protection the pot can't cover shows “Can't afford” and is disabled. Reset house clears the wallet, protection selections, outcomes, and tallies for a fresh $50,000 run. The $50k pot is smaller than the $84k total unprotected damage, so a reckless run cannot finish solvent. Testing all five risks while still solvent opens a one-time “Your savings survived!” win panel (savings left + damage avoided); damage avoided is the $84k unprotected baseline minus resolved repair damage and excludes protection purchase prices. Dropping to $0 or below opens a one-time “Wiped out!” panel (total damage taken + negative balance). Both wait for the open result panel to be acknowledged. Production build and direct store math checks pass, including a $25,000 loss taking $5,000 to −$20,000 and $72,500 avoided with all protections. Target-browser smoke testing also passes. |
| Reset for repeat demos | Done | Clears triggered disasters, damage, prevention selections, savings purchases/progress, and stale disaster-hover indicators, invalidates unresolved result callbacks from the prior run, then restores the full $50,000 savings pot. Direct reset/retrigger regression checks pass. |
| Sound effects | Done | Hail, fire, fire-loop, and tree assets are connected; the tree-fall cue is suppressed when the hazardous tree has already been removed, and clicking that stump as the first tested risk leaves the calm bird ambience playing. Disaster clips no longer preload during the initial page load; they warm on the first onboarding gesture so authored event playback remains unchanged. Water loss and the electrical arc use seeded procedural effects: their unprotected versions loop until reset, while their protected versions stop at the authored shutoff or breaker-trip beat. Smoke-alarm demand is tracked independently for the kitchen and electrical fires, so one event ending cannot silence another fire that still requires the shared detector. The approved demo intentionally has no victory or prevention-selection sound. Target-browser audio testing passes. |
| First-load introduction | Done | The two-step “Break My House” onboarding is written for homeowners rather than insurance professionals. Its overview leads with the $50,000 rainy-day savings model, explains that uninsured repair bills and prevention purchases drain that balance, and previews the illustrative insurance comparison shown after each loss. A three-step sequence makes the goal explicit: choose prevention, test all five risks, and stay above $0. The second step names every upgrade, the risk it protects, and its practical benefit. A concise, company-neutral disclosure beneath the final start action identifies the experience as a hackday educational prototype rather than an official insurance product, policy description, coverage determination, or insurance offer. A persistent “How to play” control reopens the briefing without changing the simulation. Production build passes; target-browser verification of the revised disclosure is pending. |
| Title/intro polish | Done | The start panel now uses the product name “Break My House,” plain-language homeowner framing, a prominent starting-savings ledger, and focused action copy. The final at-screen-size visual pass passes. |
...
- The house reads as a roomy three-bedroom, one-bath dollhouse: the existing
  ...
  A framed State Farm poster above the kitchen sink adds a small wall-art easter
  egg and fades with the north wall during the cutaway.
...
Ready for presentation. The full manual verification checklist passes on the
target browser, including the power-strip sequence, furnished wing and cutaway,
onboarding, prevention comparisons, audio timing, savings math, and reset flow.
The approved demo intentionally has no victory or prevention-selection sound.
There are no known presentation blockers.
...
- [x] Direct hail-result math check passes: $12,000 / $3,500 repair
  scenarios produce $11,000 / $2,500 potential insurer payments with the
  fixed $1,000 demo deductible, and the reduced outcome reports $8,500 in
  avoided damage.
- [x] Production output separates the hail, fire, and electrical visual effects
  from the initial JavaScript entry; the inactive water effect remains eager so
  its first-frame response is unchanged.
- [x] Trigger hail, reset before its result, and immediately trigger hail again;
  confirm the stale pre-reset callback cannot open a result or charge the new
  savings pot, and the new run resolves exactly once.
- [x] Set the savings balance to $5,000 and resolve the $25,000 unprotected
  kitchen fire; confirm the full charge is applied and savings become −$20,000.
- [x] On a fresh load, review both “Break My House” onboarding panels: confirm the first
...
- [x] With sound enabled, click roof, stove, bathroom pipe, power strip, and tree
  and confirm one correct sound each. Confirm the protected electrical crackle
  ends with the breaker trip while the unprotected crackle continues until reset,
  and that smoke detectors without AFCI protection begin the smoke
  alarm with the first electrical flame, not the initial arc. The approved demo intentionally has
  no victory or prevention-selection sound.
- [x] With smoke detectors enabled and AFCI disabled, trigger the power strip,
...
- [x] Click roof, observe hail stopping at both roof surfaces (including while
...
- [x] For each disaster outcome, verify the result panel clearly distinguishes
  ...
  remains reachable.
- [x] Trigger any disaster and, before pressing “Got it,” verify the other
...
- [x] Click stove, repeat with smoke detectors + extinguisher; verify the fire
...
- [x] Verify the west-wall TV is clearly separated from the stove, the sofa
  ...
  Confirm the framed State Farm kitchen poster is readable above the sink,
  remains clear of the stove interaction, and fades with the north wall instead
  of floating during the cutaway.
- [x] Click the bathroom supply line, then reset and repeat with leak sensor +
...
- [x] Click the overloaded primary-bedroom power strip, then reset and repeat
...
- [x] Inspect the furnished rooms from the default and overhead angles: verify
  ...
  silhouette.
- [x] Click tree and verify its hard roof-contact shake, lateral settling wobble,
  ...
  ambience.
- [x] Orbit above and around the house; verify the cutaway keeps all three
...
- [x] Orbit slowly across every cardinal and diagonal angle at low, middle, and
...
- [x] Verify all three bedrooms and the bathroom have visible framed doors,
  ...
  pitch, and zoom.
- [x] Orbit slowly around the lower gable and its junction with the original
...
- [x] Follow the central hall from the original living space into the wing;
...
- [x] Confirm the tree still strikes the original north-west roof opening and
  that none of the bedroom-wing geometry changes its fall or impact debris.
- [x] Reset after each flow; verify damage and prevention choices clear, savings
