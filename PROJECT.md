...
| Sims-style cutaway | Partial | Exterior walls, interior partitions, and doors use smooth angle/pitch/zoom opacity. Both roof sections now use the same pitch/zoom opacity function; the wing update tolerates partially mounted refs, meets the original eave at one non-overlapping seam, disables depth writes, and omits its prior thresholded shadow. Production build passes; manual orbit verification remains. |
